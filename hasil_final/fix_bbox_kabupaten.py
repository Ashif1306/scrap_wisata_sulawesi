"""
Fix kabupaten yang salah menggunakan pendekatan statistik:
- Hitung median lat/long per kabupaten
- Flag entry yang > 50km dari median (outlier)
- Koreksi kabupaten berdasarkan koordinat menggunakan bounding box resmi
"""
import pandas as pd
import math
import sys
...
# Sumber: Perkiraan wilayah resmi berdasarkan peta administratif
KAB_BBOX = {
    # Sulawesi Selatan
    ...
    "Kabupaten Luwu":                       (-3.25, -2.25, 120.15, 121.15),
    "Kabupaten Luwu Utara":                 (-2.65, -1.35, 119.95, 121.30),
    "Kabupaten Luwu Timur":                 (-2.90, -1.95, 120.70, 121.90),
    ...
    "Kabupaten Poso":                       (-2.10, -0.70, 120.10, 121.20),
    "Kabupaten Tojo Una-Una":               (-1.40,  0.05, 121.00, 122.40),
...
def find_correct_kab(lat, lon):
    """Cari kabupaten yang bounding box-nya mencakup koordinat ini."""
    matches = []
...
def main():
    ...
    for idx, row in df.iterrows():
        ...
        # Cek apakah koordinat masuk bounding box kabupaten
        if not (lat_min <= lat <= lat_max and lon_min <= lon <= lon_max):
            # Cari kabupaten yang benar
            ...
            outliers.append({
                ...
                'lat': lat,
                'lon': lon,
                'kab_benar': correct_candidates
            })

    print(f"\nTotal outlier (koordinat di luar bbox kabupaten): {len(outliers)}")
    print("=" * 70)

    fixes = 0
    for o in outliers:
        idx = o['idx']
        kandidat = o['kab_benar']
        print(f"\n[OUTLIER] {o['nama']}")
        print(f"  Kabupaten Salah: {o['kab_salah']}")
        print(f"  Koordinat: {o['lat']:.4f}, {o['lon']:.4f}")
        
        if len(kandidat) == 1:
            new_kab = kandidat[0]
            new_prov = KAB_TO_PROV.get(new_kab, df.at[idx, 'provinsi'])
            print(f"  FIX -> {new_kab} ({new_prov})")
            df.at[idx, 'kabupaten'] = new_kab
            df.at[idx, 'provinsi'] = new_prov
            fixes += 1
        elif len(kandidat) > 1:
            print(f"  Kandidat: {kandidat} -- perlu cek manual")
        else:
            print(f"  Tidak ada bbox yang cocok -- koordinat mungkin di laut/luar Sulawesi")

    print(f"\n{'='*70}")
    print(f"Total fix: {fixes}")

    if fixes > 0:
        # Final pass: selaraskan provinsi
        for idx, row in df.iterrows():
            kab = str(row['kabupaten']).strip()
